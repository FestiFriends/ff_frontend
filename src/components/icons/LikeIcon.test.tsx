...
describe('LikeIcon', () => {
  ...
  describe('SVG 구조 테스트', () => {
    ...
    it('각 타입별로 서로 다른 clipPath ID를 사용한다', () => {
      ...
      expect(
        emptyContainer.querySelector('#clip0_22_260')
      ).not.toBeInTheDocument();

      // emptyWhite 타입
      const { container: emptyWhiteContainer } = render(
        <LikeIcon type='emptyWhite' />
      );
      expect(
        emptyWhiteContainer.querySelector('#clip0_22_260')
      ).toBeInTheDocument();
      expect(
        emptyWhiteContainer.querySelector('#clip0_22_262')
      ).not.toBeInTheDocument();

      // active 타입
      const { container: activeContainer } = render(<LikeIcon type='active' />);
      expect(
        activeContainer.querySelector('#clip0_22_260')
      ).toBeInTheDocument();
      expect(
        activeContainer.querySelector('#clip0_22_262')
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe('렌더링 분기 테스트', () => {
    it('type에 따라 다른 SVG 컴포넌트가 렌더링된다', () => {
      ...
      expect(emptyPath?.getAttribute('d')).not.toBe(
        activePath?.getAttribute('d')
      );
    });
    ...
    it('empty가 아닌 타입들은 동일한 SVG 구조를 사용한다', () => {
      ...
      // 같은 path d 속성을 가져야 함
      expect(emptyWhitePath?.getAttribute('d')).toBe(
        activePath?.getAttribute('d')
      );
    });
  });
